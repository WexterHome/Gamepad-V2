...
void loop() {
  ...
  if(bleGamepad.isConnected()){
    ...
    leftVrxJoystickValue = map(leftVrxJoystickLecture, 0, 4095, 32737, 0);
    leftVryJoystickValue = map(leftVryJoystickLecture, 0, 4095, 32737, 0);
    rightVrxJoystickValue = map(rightVrxJoystickLecture, 0, 4095, 32737, 0);
    rightVryJoystickValue = map(rightVryJoystickLecture, 0, 4095, 32737, 0);
...
/*
void initializeDPAD(void){
  pinMode(UP_DPAD, INPUT_PULLUP);
  pinMode(RIGHT_DPAD, INPUT_PULLUP);
  pinMode(LEFT_DPAD, INPUT_PULLUP);
  pinMode(DOWN_DPAD, INPUT_PULLUP);
}

void DPADHandler(void){
  if(!digitalRead(UP_DPAD)) bleGamepad.setHats(UP_DPAD, UP_DPAD, UP_DPAD, UP_DPAD);
  else  bleGamepad.setHat();

  if(!digitalRead(RIGHT_DPAD)) bleGamepad.setHats(RIGHT_DPAD, RIGHT_DPAD, RIGHT_DPAD, RIGHT_DPAD);
  else  bleGamepad.setHats();
  
  if(!digitalRead(LEFT_DPAD)) bleGamepad.setHats(LEFT_DPAD, LEFT_DPAD, LEFT_DPAD, LEFT_DPAD);
  else  bleGamepad.setHats();
  
  if(!digitalRead(DOWN_DPAD)) bleGamepad.setHats(DOWN_DPAD, DOWN_DPAD, DOWN_DPAD, DOWN_DPAD);
  else  bleGamepad.setHats();
}
*/
